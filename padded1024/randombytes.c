...
// padded1024.SetTestRNG before invoking GenerateKey or Sign.
//
...
extern int go_falcon_padded1024_randombytes(uint8_t *out, size_t n);

int randombytes(uint8_t *out, size_t n) {
    return go_falcon_padded1024_randombytes(out, n);
}
