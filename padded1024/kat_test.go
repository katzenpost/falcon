...
//go:embed testdata/PQCsignKAT_falcon-padded-1024.rsp
var canonicalKAT []byte

// canonicalKATSha256 is the SHA-256 of testdata/PQCsignKAT_falcon-padded-1024.rsp.
// It is also the value published by Open Quantum Safe's liboqs at
//
//   tests/KATs/sig/kats.json
//
// in the "Falcon-padded-1024" -> "all" field, and represents the 100-case
...
const canonicalKATSha256 = "907a4931ddc2ce8360478a45f1bffededd6a04015b00233ecd851a62ecba06c1"
...
// Falcon-padded-1024 using our wrapper and asserts the bytes match the
...
type katCase struct {
	...
	mlen  int
	msg   []byte
	pk    padded1024.PublicKey
	sk    padded1024.PrivateKey
	sig   []byte
}
...
func buildKAT(t *testing.T) []byte {
	...
	for i := range cases {
		...
		sig, err := padded1024.Sign(&sk, cases[i].msg)
		restore()
		if err != nil {
